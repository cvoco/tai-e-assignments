...
public class Solver {
    ...
    public void addWorkList(CSVar csVar, CSObj csObj) {
        workList.addEntry(csVar, PointsToSetFactory.make(csObj));
    }

    public void addWorkList(CSVar csVar, PointsToSet pointsToSet) {
    ...
    private class StmtProcessor implements StmtVisitor<Void> {
        ...
        public Void visit(New newStmt) { // x = new C()
        ...
        public Void visit(StoreField storeField) { // x.f = y
        ...
        public Void visit(LoadField loadField) { // y = x.f
        ...
        public Void visit(Invoke invoke) {
            ...
            if (callGraph.addEdge(new Edge<>(CallKind.STATIC, csCallSite, csCallee))) {
                ...
                taintAnalysis.checkSourceCalls(csCallSite);
                taintAnalysis.checkEdges(csCallSite);
                taintAnalysis.checkSinkCalls(csCallSite);
                ...
                if (callSiteRetVar != null) {
                    CSVar callSiteReturnVar = csManager.getCSVar(context, callSiteRetVar);
                    for (Var calleeReturnVar : ir.getReturnVars()) {
                        CSVar csCalleeRetVar = csManager.getCSVar(calleeContext, calleeReturnVar);
                        addPFGEdge(csCalleeRetVar, callSiteReturnVar);
                    }
                }
            }
            return null;
    ...
    private PointsToSet propagate(Pointer pointer, PointsToSet pointsToSet) {
        ...
        for (var csObj : pointsToSet) {
            if (pointerToSet.addObject(csObj)) {
                diff.addObject(csObj);
            }
        }
        if (!diff.isEmpty()) {
            for (var succsPointer : pointerFlowGraph.getSuccsOf(pointer)) {
                workList.addEntry(succsPointer, diff);
            }
            if (pointer instanceof CSVar csVar) {
                taintAnalysis.propagate(csVar, diff);
            }
        }
        return diff;
    }
    ...
    private void processCall(CSVar recv, CSObj recvObj) {
        ...
        for (Invoke invoke : var.getInvokes()) {
            ...
            if (callGraph.addEdge(new Edge<>(CallGraphs.getCallKind(invoke), csCallSite, csCallee))) {
                addReachable(csCallee);
                // check and mark source call
                taintAnalysis.checkSourceCalls(csCallSite);
                // check and add transfer edge
                taintAnalysis.checkEdges(csCallSite);
                // check and mark sink call
                taintAnalysis.checkSinkCalls(csCallSite);
                for (int i = 0; i < argList.size(); i++) {
